...
class SemanticMapDataset(Dataset):
    def __init__(self, data_dirs, transform=None, target_transform=None):
        self.train_x = np.empty((0, 32, 32, 11))
...
def run_one_image(x, target, model, epoch=None):
    ...
    loss, y, mask = model(x, target, mask_ratio=0.75)
    x = x.detach().cpu()
...
def main(rank, world_size):
    ...
    len_train_data = int((len(data_dirs) - 1) * 0.5)
    len_val_data = (len(data_dirs) - 1) - len_train_data
    train_data_dirs = random.sample(data_dirs, len_train_data)
    val_data_dirs = [x for x in data_dirs if x not in train_data_dirs]
    test_data_dirs = [val_data_dirs.pop()]
    train_data_dirs.sort()
    val_data_dirs.sort()
    # train_data_dirs = [train_data_dirs.pop()]
    # val_data_dirs = [val_data_dirs.pop()]
